...
public class Request {
     ...
     */
    protected Request(OkHttpClient okHttpClient,
                      Executor executor,
                      okhttp3.Request request,
                      Endpoint endpoint,
                      Endpoint.ReturnType returnType,
                      JSONObject transmissionData) {
    ...
    public IResponse execute() {
        try (Response response = okHttpClient.newCall(request).execute()) {
            ...
            logger.debug("Failed to match response type of header (" + response
                    .header("Content-Type") + ") to the expected return type (" + returnType.getContentType() + ")");
            return new ResponseError("Received bad response from api: Requested return type " + returnType + " but received " + response
                    .header("Content-Type"));
        }
        catch (Exception e) {
                ...
                try (response) {
                    ...
                    if (response.code() < 200 || response.code() > 299) {
                        if (onError != null) {
                            executor.execute(() -> onError
                                    .accept(new ResponseError("Received bad response from api: Status code " + response
                                            .code())));
                            return;
                        }
                    }
                    if (response.header("Content-Type") == null) {
                        if (onError != null) {
                            logger.debug("Content-Type header is missing");
                            executor.execute(() -> onError
                                    .accept(new ResponseError("Received bad response from api: no content type")));
                            return;
                        }
                    }
                    if (response.header("Content-Type").toLowerCase(Locale.ROOT)
                            .startsWith(returnType.getContentType())) {
                        ...
                        logger.debug("Failed to match response type of header (" + response
                                .header("Content-Type") + ") to the expected return type (" + returnType
                                .getContentType() + ")");
                        executor.execute(() -> onError
                                .accept(new ResponseError("Received bad response from api: Requested return type " + returnType + " but received " + response
                                        .header("Content-Type"))));
                    }
                }
            }
    ...
    public static class Builder {
        ...
        public Builder addTransmissionData(JSONObject transmissionData) {
            ...
            else if (transmissionData == null || !JSONMatcher
                    .structureMatch(transmissionData, endpoint.getRequiredData())) {
            ...
            switch (endpoint.getRequestMethod()) {
                ...
                case POST:
                    if (transmissionData != null) {
                        reqBuilder.post(RequestBody.create(transmissionData.toString().getBytes(StandardCharsets.UTF_8),
                ...
                case PUT:
                    if (transmissionData != null) {
                        reqBuilder.put(RequestBody.create(transmissionData.toString().getBytes(StandardCharsets.UTF_8),
                                MediaType.get("application/json")));
                    }
                    else {
                ...
                case DELETE:
                    if (transmissionData != null) {
                        reqBuilder.delete(RequestBody
                                .create(transmissionData.toString().getBytes(StandardCharsets.UTF_8),
                                        MediaType.get("application/json")));
                    }
                    else {
                        reqBuilder.delete();
